...
struct AcesInternalData {
  ...
  std::unique_ptr<AcesDiagnosticManager>
  ...
  bool kokkos_initialized_here =
      false;  ///< Flag to track if this component initialized Kokkos.
};
 ...
 */
static DualView3D GetDualView(ESMC_State state, const std::string& name, int nx,
                              int ny, int nz) {
  ESMC_Field field;
  ...
  Kokkos::View<double***, Kokkos::LayoutLeft, Kokkos::DefaultExecutionSpace>
      device_view("device_" + name, nx, ny, nz);
...
void Initialize(ESMC_GridComp comp, ESMC_State /*importState*/,
  ...
  if (comp.ptr != nullptr) {
    ...
    for (const auto& scheme_config : data->config.physics_schemes) {
      ...
      if (scheme) {
        scheme->Initialize(scheme_config.options,
                           data->diagnostic_manager.get());
        data->active_schemes.push_back(std::move(scheme));
      } else {
        std::cerr
            << "ACES_Initialize: Warning - Failed to create physics scheme: "
    ...
    for (auto const& [internal_name, external_name] :
         data->config.met_mapping) {
      NUOPC_Advertise(importState_real, external_name.c_str(),
                      external_name.c_str());
    }
    ...
    for (auto const& [species, layers] : data->config.species_layers) {
      ...
      NUOPC_Advertise(exportState_real, export_name.c_str(),
                      export_name.c_str());
    }
...
void Run(ESMC_GridComp comp, ESMC_State importState, ESMC_State exportState,
  ...
  for (auto const& [species, layers] : data->config.species_layers) {
    std::string export_name = "total_" + species + "_emissions";
    if (data->export_state.fields.find(export_name) ==
        data->export_state.fields.end()) {
  ...
  if (data->default_mask.extent(0) != (size_t)nx ||
      data->default_mask.extent(1) != (size_t)ny ||
      data->default_mask.extent(2) != (size_t)nz) {
    data->default_mask =
        Kokkos::View<double***, Kokkos::LayoutLeft,
                     Kokkos::DefaultExecutionSpace>("default_mask", nx, ny, nz);
  ...
  for (auto const& [species, layers] : data->config.species_layers) {
    for (const auto& layer : layers) {
      ...
      std::copy_if(layer.scale_fields.begin(), layer.scale_fields.end(),
                   std::inserter(esmf_fields_set, esmf_fields_set.end()),
                   [&](const std::string& sf) {
                     return cdeps_fields.find(sf) == cdeps_fields.end();
                   });
  ...
  std::vector<std::string> esmf_fields(esmf_fields_set.begin(),
                                       esmf_fields_set.end());
  ...
  data->ingestor.IngestMeteorology(importState, external_esmf_fields,
                                   data->import_state, nx, ny, nz);

  // 2. Emissions from CDEPS
  if (!data->config.cdeps_config.streams.empty()) {
    data->ingestor.IngestEmissionsInline(data->config.cdeps_config,
                                         data->import_state, nx, ny, nz);
  }
  ...
  AcesStateResolver resolver(data->import_state, data->export_state,
                             data->config.met_mapping);
  ComputeEmissions(data->config, resolver, nx, ny, nz, data->default_mask, hour,
                   day_of_week);
  Kokkos::Profiling::popRegion();
