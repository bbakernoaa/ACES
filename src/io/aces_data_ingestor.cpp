...
static DualView3D CreateDualViewFromESMF(ESMC_State state, const char* name,
  ...
  Kokkos::View<double***, Kokkos::LayoutLeft, Kokkos::DefaultExecutionSpace>
      device_view(std::string("device_") + name, nx, ny, nz);
  return DualView3D(device_view, host_view);
}

void AcesDataIngestor::IngestMeteorology(
    ESMC_State importState, const std::vector<std::string>& field_names,
...
void AcesDataIngestor::InitializeCDEPS(const AcesCdepsConfig& config) {
  ...
  for (size_t i = 0; i < config.streams.size(); ++i) {
    ...
    stream_file << "stream_data_files" << id << ": " << s.file_path
                << std::endl;
    stream_file << "stream_data_variables" << id << ": " << s.name << " "
                << s.name << std::endl;
  }
  stream_file.close();
...
void AcesDataIngestor::IngestEmissionsInline(const AcesCdepsConfig& config,
  ...
  for (const auto& s : config.streams) {
    if (aces_state.fields.find(s.name) == aces_state.fields.end()) {
      ...
      Kokkos::View<double***, Kokkos::LayoutLeft, Kokkos::DefaultExecutionSpace>
          device_view("device_" + s.name, nx, ny, nz);
