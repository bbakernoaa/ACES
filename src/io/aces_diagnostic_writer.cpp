...
DualView3D AcesDiagnosticManager::RegisterDiagnostic(const std::string& name,
...
static void WriteField(ESMC_Field field, const std::string& name) {
  ESMC_FieldWrite(field, (name + ".nc").c_str(), name.c_str(), 1,
                  ESMC_FILESTATUS_REPLACE, 1, ESMF_IOFMT_NETCDF);
}

void AcesDiagnosticManager::WriteDiagnostics(const DiagnosticConfig& config,
                                             ESMC_Clock clock,
  ...
  if (config.grid_type == "gaussian") {
    ...
    int rc;
    target_mesh =
        ESMC_MeshCreateFromFile(config.grid_file.c_str(), ESMC_FILEFORMAT_SCRIP,
  ...
  for (const auto& name : config.variables) {
    ...
    if (target_grid.ptr != nullptr || target_mesh.ptr != nullptr) {
      std::cout << "ACES_Diagnostic: Interpolating '" << name << "' to "
                << config.grid_type << "..." << std::endl;
