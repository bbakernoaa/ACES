...
void MeganScheme::Run(AcesImportState& import_state,
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(int i, int j, int k) {
        ...
        double gamma_par = get_gamma_par_pceea(pardr(i, j, k), pardf(i, j, k),
                                               PAR_AVG, sc, doy);
        double gamma_co2 = get_gamma_co2(CO2A);

        double megan_emis =
            NORM_FAC * AEF_ISOP * gamma_lai * gamma_co2 *
