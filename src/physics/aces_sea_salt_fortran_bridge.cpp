...
extern "C" {
void run_sea_salt_fortran(double* u10m_ptr, double* tskin_ptr, double* sala_ptr,
                          double* salc_ptr, int nx, int ny, int nz);
}
...
void SeaSaltFortranScheme::Run(AcesImportState& import_state,
  ...
  run_sea_salt_fortran(dv_u10.view_host().data(), dv_tskin.view_host().data(),
                       sala_ptr, salc_ptr, nx, ny, nz);
