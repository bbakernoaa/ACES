...
extern "C" {
void run_dust_fortran(double* u10m, double* gwet, double* sand,
                      double* dust_emis, int nx, int ny, int nz);
}
...
void DustFortranScheme::Run(AcesImportState& import_state,
  ...
  run_dust_fortran(dv_u10.view_host().data(), dv_gwet.view_host().data(),
                   dv_sand.view_host().data(), dv_dust.view_host().data(), nx,
                   ny, nz);
