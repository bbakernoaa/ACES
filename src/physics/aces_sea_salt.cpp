...
void SeaSaltScheme::Run(AcesImportState& import_state,
  ...
  if (it_sala != export_state.fields.end()) {
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(int i, int j, int k) {
          ...
          double scale = 0.329 + 0.0904 * sst - 0.00717 * sst * sst +
                         0.000207 * sst * sst * sst;
          ...
          for (double r = r_sala_min; r < r_sala_max; r += dr) {
            ...
            total_kg += n_particles * (4.0 / 3.0 * pi *
                                       std::pow(r_mid * 1.0e-6, 3) * ss_dens);
          }
