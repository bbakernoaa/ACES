...
extern "C" {
void run_megan_fortran(double* temp, double* lai, double* pardr, double* pardf,
                       double* suncos, double* isop, int nx, int ny, int nz);
}
...
void MeganFortranScheme::Run(AcesImportState& import_state,
  ...
  if (it_temp == import_state.fields.end() ||
      it_isop == export_state.fields.end() ||
  ...
  run_megan_fortran(dv_temp.view_host().data(), dv_lai.view_host().data(),
                    ...
                    dv_suncos.view_host().data(), dv_isop.view_host().data(),
                    nx, ny, nz);
