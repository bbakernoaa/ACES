...
extern "C" {
void run_lightning_fortran(double* conv_depth, double* light_nox, int nx,
                           int ny, int nz);
}

namespace aces {

void LightningFortranScheme::Initialize(
    const YAML::Node& /*config*/, AcesDiagnosticManager* /*diag_manager*/) {
  std::cout << "LightningFortranScheme: Initialized." << std::endl;
}

void LightningFortranScheme::Run(AcesImportState& import_state,
                                 AcesExportState& export_state) {
  auto it_conv_depth = import_state.fields.find("convective_cloud_top_height");
  auto it_light_emis =
      export_state.fields.find("total_lightning_nox_emissions");

  if (it_conv_depth == import_state.fields.end() ||
      it_light_emis == export_state.fields.end())
    return;
  ...
  run_lightning_fortran(dv_conv_depth.view_host().data(),
                        dv_light_emis.view_host().data(), nx, ny, nz);
