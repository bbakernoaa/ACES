...
extern "C" {
void run_legacy_fortran(double* temp, double* wind, double* nox, int nx, int ny,
                        int nz);
}
 ...
 */
void FortranBridgeExample::Run(AcesImportState& import_state,
  ...
  if (it_temp == import_state.fields.end() ||
      it_wind == import_state.fields.end() ||
