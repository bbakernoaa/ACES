 ...
 * It applies a branchless formula to combine layers based on whether they
 * should 'add' to or 'replace' the current total for each grid cell.
 *
 * Formula: Total = Total * (1.0 - replace_flag * Mask) + (Field * Scale * Mask)
 *
 * @param config The ACES configuration containing species and layer
 * definitions.
 * @param resolver A FieldResolver to bridge between the compute engine and data
 * sources (like ESMF).
...
void ComputeEmissions(
  ...
  if (!default_mask.data()) {
    default_mask = Kokkos::View<double***, Kokkos::LayoutLeft,
  ...
  for (auto const& [species, layers] : config.species_layers) {
    ...
    if (total_view.data() == nullptr) {
      std::cerr << "ACES_Compute: Warning - Could not resolve export field "
                << export_name << std::endl;
      continue;
    }
    ...
    // Sort ALL layers for this species by hierarchy (ascending) globally across
    // categories. This ensures that higher-hierarchy layers can correctly
    // 'replace' lower-hierarchy layers regardless of which category they belong
    // to, enabling true HEMCO-style overrides.
    ...
    for (auto const& layer : sorted_layers) {
      auto field_view =
          resolver.ResolveImportDevice(layer.field_name, nx, ny, nz);
      ...
      if (!layer.diurnal_cycle.empty()) {
        ...
        if (it != config.temporal_cycles.end() &&
            it->second.factors.size() == 24) {
      ...
      if (!layer.weekly_cycle.empty()) {
        ...
        if (it != config.temporal_cycles.end() &&
            it->second.factors.size() == 7) {
          scale *= it->second.factors[day_of_week % 7];
        }
      }

      // Use a fixed-size array for scales to avoid std::vector capture in
      // lambda.
      ...
      // We apply directly to total_view to support global hierarchy-based
      // overrides.
      Kokkos::parallel_for(
          "EmissionLayerKernel",
